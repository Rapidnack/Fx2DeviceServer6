...
namespace Fx2DeviceServer
{
	public abstract class AvalonPacketBase : IAvalonPacket
	{
		...
		public byte[] WritePacket(uint addr, byte data)
		{
			byte[] array = new byte[] { data };
			return WritePacket(addr, array, 0, array.Length, false);
		}

		public byte[] WritePacket(uint addr, ushort data)
		{
			ushort[] array = new ushort[] { data };
			return WritePacket(addr, array, 0, array.Length, false);
		}
		...
		public byte[] WritePacket(uint addr, ushort[] array, int start, int length, bool isIncremental = false)
		{
			if (array.Length == 0)
				return new byte[0];
			if (start < 0 || array.Length < start + length)
		...
		public byte ReadBytePacket(uint addr)
		{
		...
		public ushort ReadUShortPacket(uint addr)
		{
		...
		public int ReadPacket(uint addr, ushort[] array, int start, int length, bool isIncremental = false)
		{
			if (array.Length == 0)
				return 0;
			if (start < 0 || array.Length < start + length)
